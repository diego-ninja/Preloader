...
class Preloader
{
    ...
    {
        if ($this->useRequire && !is_null($this->autoloader) && !file_exists($this->autoloader)) {
    ...
    {
        $contents = file_get_contents(static::STUB_LOCATION);
        if ($contents === false) {
            throw new RuntimeException('Preloader stub file not found.');
        }

        $this->compiler->list            = $this->getList();
        $this->compiler->autoloader      = $this->autoloader;
        $this->compiler->contents        = $contents;
